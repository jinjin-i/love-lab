...
export default function KakaoPage() {
  ...
  function parseResult(raw: string) {
    const parts = raw.split(/\[([^\]]+)\]/g).filter((s: string) => s.trim())
      ...
      <div style={{ flex: 1, padding: '24px 24px 5rem', overflowY: 'auto' }}>
          ...
          <>
            {limitReached ? (
              ...
              <>
                ...
                <button
                  onClick={() => setConversation('나: 오늘 뭐해?\n상대: 그냥 집에 있어\n나: 나 오늘 근처 왔는데 ㅋㅋ\n상대: 아 그래?\n나: 같이 밥 먹을래?\n상대: 오늘은 좀 피곤해서..\n나: 아 그렇구나 다음에 보자\n상대: ㅇㅇ')}
        ...
        {step === 'ad' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            {adClicked ? (
              <button onClick={startAnalysis} style={{ width: '100%', height: 52, background: 'linear-gradient(135deg, #b8922e, #e8c96a)', border: 'none', borderRadius: 14, fontSize: 15, fontWeight: 700, color: '#1a1410', cursor: 'pointer', fontFamily: 'inherit' }}>
                🔍 분석 결과 보기
              </button>
            ) : (
